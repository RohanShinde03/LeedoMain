...
import com.tribeappsoft.leedo.salesPerson.salesHead.siteVisits.adapter.SiteVisitListRecyclerAdapter;
import com.tribeappsoft.leedo.salesPerson.salesHead.siteVisits.filter.SiteVisitFilter_Activity;
...
public class SiteVisitList_Activity extends AppCompatActivity {

    @BindView(R.id.sr_selectSiteVisit) SwipeRefreshLayout swipeRefresh;
    @BindView(R.id.rv_selectSiteVisit) RecyclerView recyclerView;
  ...
  //  @BindView(R.id.mBtn_selectSiteVisit_assign) MaterialButton mBtn_assignLeads;
    @BindView(R.id.ll_selectSiteVisit_noData) LinearLayoutCompat ll_noData;
    @BindView(R.id.pb_selectSiteVisit) ContentLoadingProgressBar pb_selectLeads;
    @BindView(R.id.ll_pbLayout) LinearLayoutCompat ll_pb;
    @BindView(R.id.tv_pbLoadingMsg) AppCompatTextView tv_loadingMsg;
    private SiteVisitListRecyclerAdapter recyclerAdapter;
    private ArrayList<SiteVisitListModel> itemArrayList, multiSelect_list,PrevSalesPersonsList;
    private SiteVisitList_Activity context;
    private ArrayList<Integer> leadIdArrayList;
    private ArrayList<Integer> PrevSalesPersonIdArrayList;
    private String TAG = "SiteVisitList_Activity",api_token="", search_text = "",from_date="",to_date="";
    private int project_id =0, sales_person_id =0,current_page =1,last_page =1,call = 0,filterCount_siteVisit=0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_sitevisit_list_);
        overridePendingTransition( R.anim.trans_slide_up, R.anim.no_change );
        ButterKnife.bind(this);
        context= SiteVisitList_Activity.this;

        if (getSupportActionBar()!=null) {
            getSupportActionBar().setDisplayOptions(ActionBar.DISPLAY_SHOW_CUSTOM);
            getSupportActionBar().setCustomView(R.layout.layout_ab_center);
            ((AppCompatTextView) getSupportActionBar().getCustomView().findViewById(R.id.tv_abs_title)).setText(getString(R.string.menu_site_visitList));
        ...
        PrevSalesPersonsList = new ArrayList<>();
...
/*                    new Animations().slideOutBottom(mBtn_assignLeads);
    ...
    private void setupRecycleView()
    {
        ...
        recyclerAdapter = new SiteVisitListRecyclerAdapter(context,itemArrayList,multiSelect_list);
    ...
    public void onResume() {
        ...
        if (sharedPreferences!=null) {
            ...
            boolean isSiteLeadTransfer = sharedPreferences.getBoolean("isSiteLeadTransfer", false);
            boolean clearFilter = sharedPreferences.getBoolean("clearFilter", false);
            editor.apply();
            if (isSiteLeadTransfer) {
                ...
                if (sharedPreferences!=null) {
                    editor = sharedPreferences.edit();
                    editor.remove("isSiteLeadTransfer");
                    editor.apply();
                }
            }
            ...
            if(isFilter) {
                ...
                if (sharedPreferences!=null) {
                    ...
                    filterCount_siteVisit = sharedPreferences.getInt("filterCount_siteVisit", 0);
                ...
                //clear fields
                project_id =sales_person_id= filterCount_siteVisit = 0;
                from_date = to_date = "";
    ...
    private void call_getAllLeadsBySalesPerson()
    {
        ...
        Observable<Response<JsonObject>> responseObservable = client.getApiService().getSiteVisitedReassignLeads(api_token, project_id,sales_person_id,from_date,to_date, limit,call* limit, search_text);
    ...
    {
        SiteVisitListModel model=new SiteVisitListModel();
        ...
        if (jsonObject.has("created_at")) model.setTagDate(!jsonObject.get("created_at").isJsonNull() ? jsonObject.get("created_at").getAsString() : Helper.getTodaysDateString() );
        ...
        if (jsonObject.has("min_call_duration")) model.setMin_call_duration(!jsonObject.get("min_call_duration").isJsonNull() ? jsonObject.get("min_call_duration").getAsString() : "" );
        if (jsonObject.has("max_call_duration")) model.setMax_call_duration(!jsonObject.get("max_call_duration").isJsonNull() ? jsonObject.get("max_call_duration").getAsString() : "" );
        ...
        //if (context!= null)
        {
            runOnUiThread(() -> {
                ...
                if (getSupportActionBar()!=null) {
                    ...
                    ((AppCompatTextView) getSupportActionBar().getCustomView().findViewById(R.id.tv_abs_title)).setText(total==0 ? getString(R.string.menu_site_visitList) : getString(R.string.menu_site_visitList) + "(" + total + ")");
                    getSupportActionBar().setHomeAsUpIndicator(R.drawable.ic_close_black_24dp);
                    getSupportActionBar().setDisplayHomeAsUpEnabled(true);
                    getSupportActionBar().setHomeButtonEnabled(true);
                }
    ...
    private void refreshFeedApi()
    {
        if (Helper.isNetworkAvailable(context))
        {
            ...
            PrevSalesPersonIdArrayList = new ArrayList<>();
            PrevSalesPersonIdArrayList.clear();
            ...
            if (sharedPreferences!=null)
            {
                ...
                editor.remove("filterCount_siteVisit");
...
//            closeIcon.setImageResource(R.drawable.ic_search_close_icon);
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId())
        {
            ...
            case (R.id.action_filter):
                startActivity(new Intent(context, SiteVisitFilter_Activity.class));
                break;
        }
    ...
    private void setupBadge() {
        ...
        if (tvFilterItemCount != null) {
            Log.e(TAG, "setupBadge: "+filterCount_siteVisit );
            if (filterCount_siteVisit == 0) {
                if (tvFilterItemCount.getVisibility() != View.GONE) {
                    tvFilterItemCount.setVisibility(View.GONE);
                }
            } else {
                tvFilterItemCount.setText(String.valueOf(Math.min(filterCount_siteVisit, 99)));
        ...
        public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
            switch (item.getItemId())
            {
                ...
                case (R.id.action_proceed):
                    startActivity(new Intent(context, SelectedSiteVisitMark_Activity.class)
                            .putParcelableArrayListExtra("multiSelect_list", multiSelect_list)
                            .putExtra("leadIdsArrayList", leadIdArrayList)
                            .putExtra("PrevSalesPersonIdArrayList", PrevSalesPersonIdArrayList)
                    );
                    break;
            }
            return false;
 ...
 @SuppressLint("UseValueOf")
    private void checkInsertRemoveSalesPersonIds(int salesPersonID, boolean value) {
        if (value) PrevSalesPersonIdArrayList.add(salesPersonID);
            //else catStringArrayList.remove(new String(subcatName));
        else PrevSalesPersonIdArrayList.remove(new Integer(salesPersonID));
    }
    ...
    private void checkArrayList()
    {
        ...
        if (leadIdArrayList!=null && leadIdArrayList.size()>0) {
            ...
            Log.e(TAG, "sales Person id's Array: "+ Arrays.toString(PrevSalesPersonIdArrayList.toArray()));
        }
