package com.tribeappsoft.leedo.admin.reports.salesHeadDashboard.filter;
...
import com.tribeappsoft.leedo.admin.reports.salesHeadDashboard.model.CPModel;
...
public class Filter_SalesHeadStats extends AppCompatActivity {

    @BindView(R.id.cl_filterSalesHeadStats) CoordinatorLayout parent;
    @BindView(R.id.ll_filterSalesHeadStats_main) LinearLayoutCompat ll_main;

    @BindView(R.id.acTv_filterSalesHeadStats_selectProject) AutoCompleteTextView acTv_selectProject;
    @BindView(R.id.tv_filterSalesHeadStats_select_SalesPerson) AutoCompleteTextView acTv__select_SalesPerson;
    @BindView(R.id.tv_filterSalesHeadStats_select_event) AutoCompleteTextView acTv__select_event;
    @BindView(R.id.tv_filterSalesHeadStats_select_leadStatus) AutoCompleteTextView acTv__select_leadStatus;
    @BindView(R.id.tv_filterSalesHeadStats_select_cp) AutoCompleteTextView acTv__select_cp;

    @BindView(R.id.edt_filterSalesHeadStats_fromDate) TextInputEditText edt_fromDate;
    @BindView(R.id.edt_filterSalesHeadStats_toDate) TextInputEditText edt_toDate;
    @BindView(R.id.mBtn_filterSalesHeadStats_clearFilter) MaterialButton mBtn_clearFilter;
    @BindView(R.id.mBtn_filterSalesHeadStats_applyFilter) MaterialButton mBtn_applyFilter;
    ...
    private ArrayList<LeadStagesModel> leadStatusModelArrayList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_filter_sales_head_stats);

        ButterKnife.bind(this);
        context= Filter_SalesHeadStats.this;
    ...
    private void init()
    {
        ...
        if (Helper.isNetworkAvailable(Objects.requireNonNull(context))) {
            ...
            new Handler().postDelayed(() -> {
                call_getAllProjects();
                call_getAllSalesPersonList();
                call_getLeadStatusList();
            },1000);
    ...
    private void call_getEvent() {
        ...
        ApiClient client = ApiClient.getInstance();
        Observable<Response<JsonObject>> responseObservable = client.getApiService().getBookingEvents(api_token,user_id,1);
    ...
    private void call_getLeadStatus()
    {
        ApiClient client = ApiClient.getInstance();
    ...
    private void setLeadStatusJson(JsonObject jsonObject){
        LeadStagesModel leadStatusModel = new LeadStagesModel();
        if (jsonObject.has("lead_stage_id")) leadStatusModel.setLead_stage_id(!jsonObject.get("lead_stage_id").isJsonNull() ? jsonObject.get("lead_stage_id").getAsInt() : 0 );
        if (jsonObject.has("lead_stage_name")) leadStatusModel.setLead_stage_name(!jsonObject.get("lead_stage_name").isJsonNull() ? jsonObject.get("lead_stage_name").getAsString() : "" );

        if(leadStatusModel.getLead_stage_name()!=null && !leadStatusModel.getLead_stage_name().trim().isEmpty()){
            leadStatusArrayList.add(leadStatusModel.getLead_stage_name());
        }
            ...
            if (sharedPreferences!=null) {
                ...
                if (isFilter)
                {
                    ...
                    if ( selected_lead_status_id!=0 && leadStatusModelArrayList!=null&& leadStatusArrayList.size()>0) {
                        ...
                        acTv__select_leadStatus.setText(leadStatusModelArrayList.get(getIndexOfListForLeadStatus(leadStatusModelArrayList, selected_lead_status_id)).getLead_stage_name());
                        selected_lead_status_name = leadStatusModelArrayList.get(getIndexOfListForLeadStatus(leadStatusModelArrayList, selected_lead_status_id)).getLead_stage_name();
                    }
    ...
    private void setLeadStatusAdapter() {
        ...
        if (leadStatusArrayList.size() >0 &&  leadStatusModelArrayList.size()>0)
        {
            ...
            adapter = new CustomerAdapter(context, leadStatusModelArrayList);
            ...
            acTv__select_leadStatus.setOnItemClickListener((AdapterView<?> parent, View view, int position, long id) -> {
                ...
                for (LeadStagesModel pojo : leadStatusModelArrayList) {
                    if (pojo.getLead_stage_name().equals(itemName)) {

                        //int id = pojo.getCompany_id(); // This is the correct ID
                        selected_lead_status_id = pojo.getLead_stage_id(); // This is the correct ID
                        selected_lead_status_name = pojo.getLead_stage_name();
                        //acTv_leadStage.setTypeface(FontAwesomeManager.getTypeface(context, FontAwesomeManager.FONTAWESOME));


                        //fixedEnquiryID+=2;
                        Log.e(TAG, "Lead Status name & id " + selected_lead_status_name +"\t"+ selected_lead_status_id);
...
*/
    private int getIndexOfListForLeadStatus(List<LeadStagesModel> list, int selected_lead_status_id)
    {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            return  IntStream.range(0, list.size())
                    .filter(i -> list.get(i).getLead_stage_id() == selected_lead_status_id)
                    .findFirst().orElse(-1);
        }
        else {
            for (int i = 0; i < list.size(); i++)
                if (list.get(i).getLead_stage_id() == selected_lead_status_id)
                    return i;
            return -1;
        }
    }
