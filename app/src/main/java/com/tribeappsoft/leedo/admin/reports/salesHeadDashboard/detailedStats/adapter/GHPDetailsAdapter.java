...
import com.tribeappsoft.leedo.admin.reports.salesHeadDashboard.detailedStats.model.GHPDetailsModel;
...
public class GHPDetailsAdapter extends RecyclerView.Adapter<GHPDetailsAdapter.GHPDetailsViewHolder> {

    private Activity context;
    private ArrayList<GHPDetailsModel> itemArrayList;
    private final Animations anim = new Animations();

    public GHPDetailsAdapter(Activity context, ArrayList<GHPDetailsModel> itemArrayList) {
    ...
    @NonNull
    @Override
    public GHPDetailsAdapter.GHPDetailsViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_layout_stat_ghp_details, parent, false);
        return new GHPDetailsViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(@NonNull GHPDetailsAdapter.GHPDetailsViewHolder holder, int position) {


        final GHPDetailsModel myModel = itemArrayList.get(position);
        ...
        holder.llGHPDetailsDetails.setOnClickListener(v -> {
            //temp
            ...
            if (myModel.isExpand())  //expanded
            {
                ...
                collapse(holder.llGhpDetailsMoreDetailsView);
            ...
            else    // collapsed
            {
                ...
                expandSubView(holder.llGhpDetailsMoreDetailsView);
    ...
    private void addSiteVisits(@NonNull GHPDetailsAdapter.GHPDetailsViewHolder holder, int position, GHPDetailsModel myModel){
        ...
        for (int i = 0; i <myModel.getSite_visits().size(); i++) {
            View rowView_sub = showBranchesView(i,myModel);
            holder.llAddSiteVisitView.addView(rowView_sub);
        }
    }

    private View showBranchesView(final int position,GHPDetailsModel myModel) {
    ...
    public static class GHPDetailsViewHolder extends RecyclerView.ViewHolder {

        @BindView(R.id.mtv_itemGhpDetails_cuIdNumber)
        MaterialTextView mtvCuIdNumber;
        @BindView(R.id.mtv_itemGhpDetails_name)
        MaterialTextView mtvFullName;
        @BindView(R.id.mtv_itemGhpDetails_mobileNumber)
        MaterialTextView mtvMobileNumber;
        @BindView(R.id.mtv_itemGhpDetails_cp_name_value)
        MaterialTextView mtvCpName;

        @BindView(R.id.mtv_itemGhpDetails_ghp_projectName)
        MaterialTextView mtvGhpProject;
        @BindView(R.id.mtv_itemGhpDetails_GHP_no)
        MaterialTextView mtvGHPNo;
        @BindView(R.id.mtv_itemGhpDetails_amount)
        MaterialTextView mtvGHPAmount;
        @BindView(R.id.mtv_itemGhpDetails_date)
        MaterialTextView mtvGHPDate;
        @BindView(R.id.ll_ghpDetails_moreDetailsView)
        LinearLayoutCompat llGhpDetailsMoreDetailsView;
        @BindView(R.id.iv_item_statGhpDetails_ec)
        AppCompatImageView iv_expandCollapseIcon;
        @BindView(R.id.ll_add_site_visit_view)
        LinearLayoutCompat llAddSiteVisitView;
        @BindView(R.id.ll_ghpDetails_site_visit)
        LinearLayoutCompat llSiteVisit;
        @BindView(R.id.ll_ghp_details)
        LinearLayoutCompat llGHPDetailsDetails;
        @BindView(R.id.ll_itemCpNameLayout)
        LinearLayoutCompat llCpNameLayout;

        public GHPDetailsViewHolder(@NonNull View itemView) {
            super(itemView);
